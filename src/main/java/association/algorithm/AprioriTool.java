...
public class AprioriTool {
    ...
    public AprioriTool(Sqlproperty sqlproperty, int minSupportCount) {
    ...
    /**
     * 从文件中读取数据存放于totalGoodsIDs
     */
    private void readDataFile() {
        Conn conn = new Conn(sqlproperty);
        //File file = new File(filePath);
        ArrayList<String[]> dataArray = conn.getData();
            ...
            for (int i = 0; i < list.size() - 1; i++) {
                ...
                for (int j = i + 1; j < list.size(); j++) {
                    ...
                    if (resultArray.length == (array1.length + 1)) {
                        isContain = isIDArrayContains(resultContainer, resultArray);
                        if (!isContain) {
     ...
     * @return
     */
    private boolean isIDArrayContains(ArrayList<String[]> container, String[] array) {
    ...
    public void printAttachRule(double minConf) {
        ...
        for (int i = 1; i < totalNum - 1; i++) {
            ...
            System.out.print(MessageFormat.format("},confidence(置信度)：{0}/{1}={2}", count2, count1, count2 * 1.0 / count1));
